 ...
 */
export async function handleChatCore({ body, modelInfo, credentials, log, onCredentialsRefreshed, onRequestSuccess, onDisconnect, clientRawRequest, connectionId, userAgent, apiKey, ccFilterNaming, rtkEnabled, headroomEnabled, headroomUrl, headroomCompressUserMessages, cavemanEnabled, cavemanLevel, ponytailEnabled, ponytailLevel, pxpipeEnabled, pxpipeMinChars, pxpipeTimeoutMs, pxpipeTransform, onPxpipeEvent, sourceFormatOverride, providerThinking, streamStallTimeoutMs }) {
  ...
  // Wins over model-level targetFormat so the user's explicit per-connection setting isn't
  // silently overridden by registry entries like { targetFormat: "claude" }.
  ...
  const targetFormat = outputFormat
    ? runtimeTransport?.format || getTargetFormat(provider, credentials)
  ...
  const sharedCtx = { provider, model, body, stream, translatedBody, finalBody, requestStartTime, connectionId, apiKey, clientRawRequest, onRequestSuccess, pxpipe: pxpipeSummary, reqTag, log, streamStallTimeoutMs };
